...
UPLOAD_BUFFER_CHARACTERISTIC_MTU = 200
...
def upload_program(roo_blocks, program, progress_cb):
    ...
    print(f'writing program to flash over ble... (size {program_size})')

    for offset in range(0, program_size, UPLOAD_BUFFER_CHARACTERISTIC_MTU):
        chunk = program[offset:offset +
                        UPLOAD_BUFFER_CHARACTERISTIC_MTU].encode()
        print('uploading', len(chunk), chunk)
    ...
    try:
        ...
        while True:
            if roo_blocks is None:
                ...
                try:
                    buffer = await asyncio.wait_for(reader.readline(), timeout=2)
                ...
                if msg['type'] == 'upload-and-play':
                    ...
                    roo_blocks.char_write(
                        DELAY_CHARACTERISTIC, struct.pack('f', delay))
                else:
